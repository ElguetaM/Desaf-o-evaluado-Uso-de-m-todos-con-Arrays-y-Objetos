...
let totalDental = dental.forEach(function (td) {
  document.write(
    `<p>${td.hora} - ${td.especialista} - ${td.paciente} - ${td.rut} - ${td.prevision}.</p>`
  );
});

//Ejercicio-4//

const pacientes = radiologia.concat(dental, traumatologia);

for (let i = 0; i < pacientes.length; i++) {
  document.write(`<p>${pacientes[i].paciente}.</p>`);
}

//Ejercicio-5//

let filtro1 = dental.filter((afiliado) => afiliado.prevision === "ISAPRE");

for (let i = 0; i < filtro1.length; i++) {
  document.write(`<p>${filtro1[i].paciente} - ${filtro1[i].prevision}.</p>`);
}

//Ejercicio-6//

let filtro2 = traumatologia.filter(
  (afiliado2) => afiliado2.prevision === "FONASA"
);

for (let i = 0; i < filtro2.length; i++) {
  document.write(`<p>${filtro2[i].paciente} - ${filtro2[i].prevision}.</p>`);
}
